...
def B():
    ...
    for j in range(0,n-1):
        print str('j is:')
        print j
        orders_in_range_j=orders_in_range[j]['id']
        print str('orders_in_range_j is:')
        print orders_in_range_j
        if orders_in_range_j==pedidos_ai_list[j]:
            print str('pedidos_ai_list[j]:')
            print pedidos_ai_list[j]
            query_b = query_ai  #el nuevo constraint ... 
            query_b &= db.po.id==pedidos_ai_list[j]['id'] # toma el pedido con id igual al j-esimo pedido de la lista devuelve el id
