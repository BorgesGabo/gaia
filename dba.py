...
db.define_table(
    ...
    Field('customer_id', 'list:reference customer'),
    format='%(po_number)s')
...
db.define_table(
    ...
    Field('product_id', 'list:reference product'),
...
# validates the customer id is referenced via format showing customer field: ful_name
# this validation assures that once you pick up one customer in po table as a name is referenced with the correspondant id in customer's table
db.po.customer_id.requires= IS_IN_DB(db,db.customer.id,'%(full_name)s')
...
# validates many to many relationship. When a product name is chosen in the form, its id is retrieved and stored to correlate the tables
db.po_detail.product_id.requires=IS_IN_DB(db,'product.id','%(name)s',multiple=True)
# validates one to many relationship. When a po number is chosen in the form, its id is retrieved and stored to correlate the tables
db.po_detail.po_id.requires=IS_IN_DB(db,'po.id','%(po_number)s')
